...
from bot_manager.game.constatnts import GameCommand, CALLBACK_DATA, WAITING_TIME, START_MESSAGE, \
    ANSWER_KEYBOARD, MIN_PLAYERS
from bot_manager.game.engine import GameEngine
from bot_manager.game.messages import Messages
from bot_manager.game.session import SessionManager
from bot_manager.game.types import GameSession, GameState, Player
from bot_manager.game.utils import extract_command
...
class GameManager:
    def __init__(
        ...
        self.waiting_timers = {}
        self.handlers: dict[GameCommand, Callable[[Message], Awaitable[None]]] = {
    ...
    async def process_message(self, message: Message):
        if message.entities is not None:
            ...
            if session and session.is_game_state(GameState.WAITING_FOR_ANSWER):
                if GameEngine(session).is_active_player(user_id):
    ...
    async def _handle_join(self, message: Message):
        ...
        if not session:
            await self._init_game(
                chat_id, message.from_.id, message.from_.username
            )
            return
        if session.is_game_state(GameState.WAITING_FOR_PLAYERS):
            await self._add_player(
                session, message.from_.id, message.from_.username
            )
            return
        await self.tg.send_message(
            MessageReply(
    ...
    async def _stop_game(self, session: GameSession):
        ...
        await self.tg.send_message(MessageReply(
            chat_id=session.chat_id,
            text=Messages.game_finished(session)
        ))

    async def _add_player(self, session: GameSession, user_id: int, username: str):
        session.add_player(user_id, username)
        await self.session_manager.set_session(session)
        await self.tg.send_message(
            MessageReply(
                chat_id=session.chat_id,
                text=Messages.player_joined(
    ...
    async def _wait_and_start(self, chat_id: int):
        ...
        if session is None or not session.is_game_state(GameState.WAITING_FOR_PLAYERS):
            return
        ...
        if session.is_ready_to_start():
            ...
            await self.tg.send_message(MessageReply(
                chat_id=chat_id,
                text=Messages.not_enough_players()
            ))
            await self.session_manager.clear_session(session.chat_id)
